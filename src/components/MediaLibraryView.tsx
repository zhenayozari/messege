      ...
      <div className="px-6 py-4 border-b border-zinc-200 flex items-center justify-between bg-white shrink-0">
        <div>
          <h1 className="text-base font-bold text-zinc-900 flex items-center gap-2">
            ...
            <span>Медиабиблиотека объектов</span>
          </h1>
        ...
        {filteredMedia.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-5">
            {filteredMedia.map((asset) => (
              <div
                key={asset.id}
                className="group bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-2xs hover:shadow-md transition-all flex flex-col"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full border border-zinc-200 p-5 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleUpload} className="space-y-3 text-xs">
              <div>
                <label className="block font-medium text-zinc-700 mb-1">Название фото / видео</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block font-medium text-zinc-700 mb-1">URL изображения / файла</label>
                <input
                  type="url"
                  placeholder="https://..."
                  value={newUrl}
